...
为证明项目是在比赛当天持续、实时开发，Git 历史必须保留清晰的开发时间线：

- 采用小步、频繁提交；每完成一个可独立说明且已验证的子目标，就创建一次真实提交；
- 提交后及时推送到 `origin/main`，不要把当天的大量开发积压到最后一次性提交或推送；
- 提交信息应准确描述本次增量，禁止为了制造频率而创建空提交、虚假提交或无意义拆分；
- 频繁提交不降低质量与安全要求：每次仍只暂存本次相关文件，并在提交前完成适合该增量的检查；
- 若当前增量暂时无法通过必要验证，应先记录问题并修复，不得把未验证状态伪装为已完成。
